...
### 1. Root & Base Horizon
`/` (Service List, Customer flow) and `/admin` (Services dashboard, Admin flow, once logged in) are each a navigation root for their respective role. Pressing the native back button from a root screen must trigger the Double-Press Exit sequence (§3), never standard history popping.

- *Linear/Browsing Screens:* `/book/:serviceId` (Time Slot Picker → Customer Details Form), `/confirmation/:appointmentId`, `/admin/login`, `/admin/appointments` — back button steps back one level through standard linear history, not a guarded root.

### 2. Modal & Sub-View Orchestration
- *Modal-First Dismissal:* Any open modal (the appointment confirm/cancel confirmation dialog, the Service create/edit modal) must intercept the back-button event and close itself first — it must never fall through to navigate the page underneath.

- *Tabbed View Handling:* No tab groups exist in v1 (Admin dashboard uses separate routes for Services vs. Appointments, not in-page tabs) — this section doesn't currently apply; revisit if a tabbed view is added later.
...
- *Toast Feedback Interception:* The first back-button press on a root screen must show a non-modal Toast: "Press back again to exit" (Hebrew: "לחץ שוב על חזרה כדי לצאת").
...
- *Logged out (or session expired via a `401`):* Back button from `/admin/login` → `/` (the public Service List), not app exit.
- *Unauthenticated (Customer) flow:* No auth branching needed for the public booking flow — navigation is purely route-based (§1) plus modal dismissal (§2), since `Customer` never logs in.

### 5. Memory Stack Safety
- *Destructive Navigation:* When the Admin successfully logs in, `replace`/stack-reset `/admin/login` out of history so the back button from `/admin` never returns to the login screen.
