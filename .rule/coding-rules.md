...
- Project: `Hila Tours` — a monorepo with a React/Vite frontend and a Node.js/Express backend (`tour-service` on port 4001, `user-management-service` on port 4002).
...
- `backend/` contains two services — `tour-service` (port 4001, env var `TOUR_SERVICE_BASE_URL`) and `user-management-service` (port 4002, env var `USER_SERVICE_BASE_URL`) — each its own Node.js/Express process with its own `package.json`. There is no gateway; the frontend calls each service's base URL directly.
- The frontend never calls a database directly; it only talks to the backend services over HTTPS.

### Frontend
- All communication with the backend goes through its REST API(s) — there is no direct DB access from the frontend.
- All API calls and business logic must be encapsulated in a service file in `src/services/` (one per domain entity: `tour`, `bus`, `busType`, `seat`, `admin`/`auth`).
...
- Each service organizes its domains under `api/<domain>/`, following the pattern:
  ...
  - `tour-service` domains: `tour/`, `bus/`, `busType/`, `seat/`.
  - `user-management-service` domains: `admin/`, `auth/`.
...
- Global state is managed exclusively via Zustand, using a sliced/module pattern in `src/store/` (one slice per feature: `tour`, `bus`, `busType`, `seat`, `auth`).
...
- `seat` is the high-contention entity: since its status can change from multiple directions at once (passenger requests, admin actions, other admins), keep its live state in a dedicated `seat.slice.ts` as the single source of truth rather than deriving it locally in a component.
