...
- Prefer singular domain terms in branch names when applicable — e.g. `task/appointment-approve-flow`, `bug/timeslot-race-condition`.
...
- For changes to `TimeSlot`'s state logic specifically, confirm the concurrency/race-condition behavior has been checked before merge — this is the highest-risk area in the codebase (see `testing-rules.md`).
...
- The four services (`api-gateway`, `appointment-service`, `catalog-service`, `user-management-service`) deploy independently — version each one separately rather than sharing a single repo-wide version number, so a breaking change in one service's API doesn't force a version bump in another.
...
- Ensure release notes summarize user-visible changes and any breaking behavior — call out explicitly if a change affects `TimeSlot`'s state contract, since the frontend and `appointment-service` must stay in sync on this.
