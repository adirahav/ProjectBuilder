...
examples:
   - input: "Create a primary button"
     output: '<Button className="bg-primary hover:bg-primary/90 text-white rounded-xl shadow-lg shadow-primary/20">Confirm Appointment</Button>'
---

# CSS Layer - 100% Tailwind CSS Architecture (v4)
*Objective:* Build high-fidelity, responsive, RTL/LTR-bidirectional UI using a pure Tailwind CSS architecture. Custom CSS/SCSS files, `@apply` directives, and BEM naming conventions are strictly prohibited.
...
ClinicBook is bilingual — Hebrew (RTL, primary) and English (LTR, secondary) — so this section applies throughout the app, not as an edge case.
...
- **No spatial-component exception:** ClinicBook has no map/floor-plan/seat-diagram component — the `TimeSlotPicker` grid is a time-ordered list, not a spatial layout, so it follows normal RTL/LTR direction like everything else. If a future feature introduces a genuinely spatial UI, revisit this section then.
...
- Map each `TimeSlotStatus`/`AppointmentStatus` value to a `@theme` token, not a raw hex value (see `.rule/style-rules.md`'s Domain-Specific Color Mapping table).
