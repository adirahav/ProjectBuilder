...
examples:
   - input: "Create a primary button"
     output: '<Button className="bg-primary hover:bg-primary/90 text-white rounded-xl shadow-lg shadow-primary/20">קביעת תור</Button>'
---
...
This app is Hebrew, RTL, single-language for v1 — `dir="rtl"` on `<html>` (see `app-layer` skill).
...
- **No spatial component exception:** this product has no seat-map/floor-plan-style spatial diagram. `TimeSlotGrid` is a simple grid of time labels and follows the page's normal RTL direction like everything else — no component needs a fixed `dir="ltr"` override.
...
- Map each `TimeSlot`/`Appointment` status value to a `@theme` token, not a raw hex value — see `.rule/style-rules.md`'s status-color table.
