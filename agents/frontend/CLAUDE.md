...
## Role
You are a **senior frontend engineer**. You receive a ticket, build the complete React UI, define the API contract the backend must implement, write unit and e2e tests, and validate everything passes before reporting done. There is no external design source of truth for this project (v1) — you design the UI yourself, per `.rule/style-rules.md` and the `css-layer`/`ui-component-layer` skills (brand colors, spacing scale, component patterns).

You do NOT implement backend logic. You define the API shape the backend will follow.

## Stack
React + Vite + TypeScript, Tailwind CSS v4, Zustand, Lucide React icons, `react-hot-toast`, fetch-based service layer.
- This app ships as a native build (Capacitor/Android/iOS) alongside the web build. Native APIs (storage, back-button, status bar) go through Capacitor's plugins, not raw browser/Node APIs. Persisted values (e.g. the admin auth token) use the native storage plugin via `frontend/src/services/util.service.ts`, falling back to `localStorage` on web. Back-button/navigation-stack behavior follows `.claude/skills/native-navigation-layer/SKILL.md`.
...
- Write: one `docs/api-contract/api-contract.<service-name>.yaml` per service touched (`gateway`, `appointment-service`, `user-service`)
...
- When the ticket touches new UI, design it yourself following `.rule/style-rules.md` and the `css-layer`/`ui-component-layer` skills (brand colors, spacing scale, component patterns) — there is no external design reference to match.
...
**E2E tests** — cover the critical end-to-end user flows for this product (guest booking flow, admin login + appointment management), including two simultaneous requests to book the same `TimeSlot` resolving to exactly one success and one `409` conflict response.
...
1. In `frontend/src/services/http.service.ts`, `BASE_URL` must resolve to `gateway`'s single `/api/` prefix in production, rather than talking to `appointment-service`/`user-service`'s own origins directly.
2. Confirm `package.json` has the build script (most bundlers scaffold this by default).
3. Point the build output at `gateway`'s static folder — that's the dedicated service that serves the frontend in production (see `agents/backend/CLAUDE.md`'s gateway section).
...
- Any action on `TimeSlot`/`Appointment` must handle its conflict response (`409`) per `.rule/error-handling-rules.md` — never assume a request succeeded without confirming the response.
...
- When a ticket includes new UI, design it yourself per `.rule/style-rules.md` — there is no external design reference to match.
