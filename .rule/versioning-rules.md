...
- Prefer singular domain terms in branch names when applicable — e.g. `task/timeslot-hold-flow`, `bug/timeslot-race-condition`.
...
- `api-gateway`, `booking-service`, `user-service`, and `notification-service` deploy independently — version each one separately rather than sharing a single repo-wide version number — a breaking change in one service's API shouldn't force a version bump in another.
...
- Ensure release notes summarize user-visible changes and any breaking behavior — call out explicitly if a change affects `TimeSlot`'s state contract, since the frontend and `booking-service` must stay in sync on this.
