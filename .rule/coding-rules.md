...
- Project: Dog Grooming Appointment Booking System — a monorepo with a React/Vite frontend and a Node.js/Express backend (`api-gateway` :4000, `booking-service` :4001, `user-service` :4002, `notification-service` :4003).
...
- `backend/` contains `api-gateway`, `booking-service`, `user-service`, and `notification-service` — each its own Node.js/Express process with its own `package.json`.
...
- All API calls and business logic must be encapsulated in a service file in `src/services/` (one per domain entity: `service.service.ts`, `timeSlot.service.ts`, `appointment.service.ts`, `auth.service.ts`).
...
- Each service organizes its domains under `api/<domain>/`, following the pattern:
  ...
  - `api-gateway` domains: `auth-proxy/`, `routing/`.
  - `booking-service` domains: `service/`, `timeSlot/`, `appointment/`.
  - `user-service` domains: `auth/`, `admin/`.
  - `notification-service` domains: `notification/`.
...
- `TimeSlot` is a contested/high-contention entity: since its status can change from multiple directions at once (another customer's hold, an expiry), keep its live state in a dedicated slice as the single source of truth rather than deriving it locally in a component.
