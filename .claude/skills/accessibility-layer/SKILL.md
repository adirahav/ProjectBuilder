...
*Objective:* Build digital experiences that are perceivable, operable, understandable, and robust for all users. This layer ensures the dog grooming clinic booking app is not just "compliant" but truly inclusive by design.
...
- *Multi-State Entities:* Both `TimeSlot` (`available`/`held`/`booked`) and `Appointment` (`pending`/`confirmed`/`cancelled`/`completed`) have a `status` that must never rely on color alone. Each status should carry a distinct icon plus a text label reachable via tooltip/`aria-label`, so the UI is usable by colorblind users and in grayscale/high-contrast display modes.
...
- *Aria-Labels:* Every icon-only button must have a descriptive, hardcoded `aria-label` in the active language (Hebrew default, English alternate — see `ui-component-layer` skill for the i18n approach).
...
- *Error Association:* Every inline validation error must be linked to its input via `aria-describedby`, so screen readers announce the error when the field receives focus — not only when it's visually next to the field. Applies to every form in the app: the guest booking contact-details form (customer name + phone number, both required, on `ContactDetailsPage` at `/book/:serviceId/:timeSlotId/confirm`) and the admin login form (email + password, on `AdminLoginPage` at `/admin/login`).
...
### 6. Directional Accessibility (RTL Hebrew default / LTR English alternate)
- *Directional Clarity:* Ensure the reading order for screen readers follows the active language's flow — RTL when Hebrew is active, LTR when English is active.
