...
- The three backend services (`gateway` :5000, `appointment-service` :5001, `user-service` :5002) deploy independently — version each one separately rather than sharing a single repo-wide version number, since a breaking change in one service's API shouldn't force a version bump in another. The frontend versions independently as well.
