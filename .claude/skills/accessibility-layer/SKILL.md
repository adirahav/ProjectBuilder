...
*Objective:* Build digital experiences that are perceivable, operable, understandable, and robust for all users. This layer ensures the Dog Grooming Appointment Booking System is not just "compliant" but truly inclusive by design.
...
- *Multi-State Entities:* `TimeSlot` (`open`/`held`/`booked`) and `Appointment` (`pending`/`confirmed`/`cancelled`) must never rely on color alone. Each status should carry a distinct icon plus a text label reachable via tooltip/`aria-label`, so the UI is usable by colorblind users and in grayscale/high-contrast display modes.
...
- *Aria-Labels:* Every icon-only button must have a descriptive, hardcoded `aria-label` in the active locale (Hebrew or English — via the phrase-table lookup, see `ui-component-layer` skill).
...
- *Error Association:* Every inline validation error must be linked to its input via `aria-describedby`, so screen readers announce the error when the field receives focus — not only when it's visually next to the field. Applies to every form in the app: the Customer Details Form (name/phone/email), the Admin Login form, and the Admin Service create/edit form.
...
### 6. Directional Accessibility (RTL default / LTR for English)
- *Directional Clarity:* Ensure the reading order for screen readers follows the active locale's flow (RTL for Hebrew, LTR for English).
