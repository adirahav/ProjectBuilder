...
- `booking-service` and `admin-service` deploy independently, so version each one separately rather than sharing a single repo-wide version number — a breaking change in one service's API shouldn't force a version bump in another.
