...
- *Data Persistence:* Managing how data is saved and retrieved (via `api-gateway`, which proxies to `booking-service` and `user-service`).
...
- Name files with `.service.ts` suffix (e.g., `service.service.ts`, `timeSlot.service.ts`, `appointment.service.ts`, `auth.service.ts`)
...
async function deactivate(id: string): Promise<any> {
    // Soft-delete via isActive: false — sets it server-side, does not remove the document
    const result = await httpService.patch<any>(`${BASE_URL}${id}/deactivate`, {})
    return result
}
```

`timeSlot.service.ts` follows the same pattern for its own named actions (`getOpenSlots`, `hold`) rather than the generic `getList`/`save`/`remove` shape above — each is its own named function calling the corresponding endpoint (see the relevant API contract file), since hold/book aren't simple CRUD.
...
describe('Service (domain) Service', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
...
- `timeSlot.service.ts`'s `hold` function must surface a `409` as a distinct case, not a generic error — let the caller (page/hook) show an actionable message and refresh. Never silently swallow or retry it inside the service.
