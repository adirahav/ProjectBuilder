...
1. **Authentication Hydration:**
   - The Dog Grooming Appointment Booking System has one authenticated role: `admin`. `customer` is never authenticated — no login/signup, so there is no session to hydrate for that role.
   - Hydration Flow:
...
3. **Routing Strategy (Auth Guard):**
   - **Public Routes (no auth required):** `/` (Service List), `/book/:serviceId` (Time Slot Picker + Customer Details Form), `/confirmation/:appointmentId` (Booking Confirmation), `/admin/login`.
   - **Private Routes (role-gated, `admin`):** `/admin` (Services management), `/admin/appointments` (Appointments dashboard).
   - Guard Logic:
...
4. **Global Layout Wrapper:**
   ...
   - Handle directionality at the HTML level: `dir="rtl"` by default (Hebrew), switching to `dir="ltr"` when English is selected.
...
6. **Version Governance & Upgrade Orchestration**
   - Not in scope for v1 — no forced-update UI.
